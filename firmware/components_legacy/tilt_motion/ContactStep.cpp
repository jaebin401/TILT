...
std::optional<RobotContactTrajectorySample> sampleContactStep(
    ...
    RobotContactTrajectorySample result{};
...
std::optional<ContactStepPoseSample> sampleContactStepPose(
    ...
    float elapsed_seconds,
    float duration_seconds) {
    const auto contact_sample = sampleContactStep(
        start,
        swing_leg,
        swing_target,
        clearance_mm,
        elapsed_seconds,
        duration_seconds);
    if (!contact_sample.has_value()) {
        return std::nullopt;
    }

    const auto pose = inverseRobotContactKinematics(
        contact_sample->position,
        left_hip_yaw_rad,
        right_hip_yaw_rad);
    if (!pose.has_value()) {
        return std::nullopt;
    }
    return ContactStepPoseSample{*pose, contact_sample->finished};
}
